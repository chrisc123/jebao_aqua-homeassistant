...
def parse_response_prefix(data: bytes) -> tuple[bytes, bytes]:
    ...
    flag = data[
        idx
    ]  # Flag is usually 0 but seems to be set to 1 for unsolicited status updates
    idx += 1
...
def set_normal_bits_in_first_16(
    av: bytearray, bit_offset: int, length_bits: int, value: int
):
...
def set_bits_in_byte(
    av: bytearray, byte_index: int, bit_offset: int, bit_length: int, value: int
):
    mask = (1 << bit_length) - 1
