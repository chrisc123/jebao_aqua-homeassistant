...
from .util import load_attribute_models  # Add this import
...
class GizwitsDataUpdateCoordinator(DataUpdateCoordinator):
    def __init__(self, hass, api):
        ...
        self._available_devices = set()  # Add this to track available devices
        ...
        async with self._device_update_locks[device_id]:
            ...
            if device_info and "lan_ip" in device_info:
                try:
                    ...
                    LOGGER.warning(
                        "Local connection failed for device %s (%s): %s",
                        device_id,
                        device_info["lan_ip"],
                        str(e),
                    )
            ...
            # Fall back to cloud API if local fails or not configured
            try:
                data = await self.api.get_device_data(device_id)
                if data:
                    return data
            except Exception as e:
                LOGGER.error("Cloud API error for device %s: %s", device_id, str(e))

            return None

    def is_device_available(self, device_id: str) -> bool:
        """Return if a specific device is available based on actual communication."""
        return device_id in self._available_devices
